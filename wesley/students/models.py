from django.urls import reverse
from django.db import models
from django.conf import settings
import datetime


STATUS_CHOICES = (
    ('대기', '대기'),
    ('재원', '재원'),
    ('휴원', '휴원'),
    ('퇴원', '퇴원'),
    ('예정', '예정'),
)

TYPE_CHOICES = (
    ('거절', '거절'),
    ('기존', '기존'),
    ('미배정', '미배정'),
    ('신규', '신규'),
    ('재입학', '재입학'),
)

TYPE_DETAIL_CHOICES = (
    ('우선 배정', '우선 배정'),
    ('다음 배정', '다음 배정'),
    ('내부 거절', '내부 거절'),
    ('외부 거절', '외부 거절'),
)

CLASS_CHOICES = (
    ('1A01', '1A01'),
    ('1A02', '1A02'),
    ('1A03', '1A03'),
    ('1A04', '1A04'),
    ('1A05', '1A05'),
    ('1A06', '1A06'),
    ('1A07', '1A07'),
    ('1A08', '1A08'),
    ('1A09', '1A09'),
    ('1A10', '1A10'),
    ('1A11', '1A11'),
    ('1A12', '1A12'),
    ('1B01', '1B01'),
    ('1B02', '1B02'),
    ('1B03', '1B03'),
    ('1B04', '1B04'),
    ('1B05', '1B05'),
    ('1B06', '1B06'),
    ('1B07', '1B07'),
    ('1B08', '1B08'),
    ('1B09', '1B09'),
    ('1B10', '1B10'),
    ('1B11', '1B11'),
    ('1B12', '1B12'),
    ('1C01', '1C01'),
    ('1C02', '1C02'),
    ('1C03', '1C03'),
    ('1C04', '1C04'),
    ('1C05', '1C05'),
    ('1C06', '1C06'),
    ('1C07', '1C07'),
    ('1C08', '1C08'),
    ('1C09', '1C09'),
    ('1C10', '1C10'),
    ('1C11', '1C11'),
    ('1C12', '1C12'),
    ('1D01', '1D01'),
    ('1D02', '1D02'),
    ('1D03', '1D03'),
    ('1D04', '1D04'),
    ('1D05', '1D05'),
    ('1D06', '1D06'),
    ('1D07', '1D07'),
    ('1D08', '1D08'),
    ('1D09', '1D09'),
    ('1D10', '1D10'),
    ('1D11', '1D11'),
    ('1D12', '1D12'),
    ('1E01', '1E01'),
    ('1E02', '1E02'),
    ('1E03', '1E03'),
    ('1E04', '1E04'),
    ('1E05', '1E05'),
    ('1E06', '1E06'),
    ('1E07', '1E07'),
    ('1E08', '1E08'),
    ('1E09', '1E09'),
    ('1E10', '1E10'),
    ('1E11', '1E11'),
    ('1E12', '1E12'),
    ('1F01', '1F01'),
    ('1F02', '1F02'),
    ('1F03', '1F03'),
    ('1F04', '1F04'),
    ('1F05', '1F05'),
    ('1F06', '1F06'),
    ('1F07', '1F07'),
    ('1F08', '1F08'),
    ('1F09', '1F09'),
    ('1F10', '1F10'),
    ('1F11', '1F11'),
    ('1F12', '1F12'),
    ('2A01', '2A01'),
    ('2A02', '2A02'),
    ('2A03', '2A03'),
    ('2A04', '2A04'),
    ('2A05', '2A05'),
    ('2A06', '2A06'),
    ('2A07', '2A07'),
    ('2A08', '2A08'),
    ('2A09', '2A09'),
    ('2A10', '2A10'),
    ('2A11', '2A11'),
    ('2A12', '2A12'),
    ('2B01', '2B01'),
    ('2B02', '2B02'),
    ('2B03', '2B03'),
    ('2B04', '2B04'),
    ('2B05', '2B05'),
    ('2B06', '2B06'),
    ('2B07', '2B07'),
    ('2B08', '2B08'),
    ('2B09', '2B09'),
    ('2B10', '2B10'),
    ('2B11', '2B11'),
    ('2B12', '2B12'),
    ('2C01', '2C01'),
    ('2C02', '2C02'),
    ('2C03', '2C03'),
    ('2C04', '2C04'),
    ('2C05', '2C05'),
    ('2C06', '2C06'),
    ('2C07', '2C07'),
    ('2C08', '2C08'),
    ('2C09', '2C09'),
    ('2C10', '2C10'),
    ('2C11', '2C11'),
    ('2C12', '2C12'),
    ('2D01', '2D01'),
    ('2D02', '2D02'),
    ('2D03', '2D03'),
    ('2D04', '2D04'),
    ('2D05', '2D05'),
    ('2D06', '2D06'),
    ('2D07', '2D07'),
    ('2D08', '2D08'),
    ('2D09', '2D09'),
    ('2D10', '2D10'),
    ('2D11', '2D11'),
    ('2D12', '2D12'),
    ('2E01', '2E01'),
    ('2E02', '2E02'),
    ('2E03', '2E03'),
    ('2E04', '2E04'),
    ('2E05', '2E05'),
    ('2E06', '2E06'),
    ('2E07', '2E07'),
    ('2E08', '2E08'),
    ('2E09', '2E09'),
    ('2E10', '2E10'),
    ('2E11', '2E11'),
    ('2E12', '2E12'),
    ('2F01', '2F01'),
    ('2F02', '2F02'),
    ('2F03', '2F03'),
    ('2F04', '2F04'),
    ('2F05', '2F05'),
    ('2F06', '2F06'),
    ('2F07', '2F07'),
    ('2F08', '2F08'),
    ('2F09', '2F09'),
    ('2F10', '2F10'),
    ('2F11', '2F11'),
    ('2F12', '2F12'),
    ('3A01', '3A01'),
    ('3A02', '3A02'),
    ('3A03', '3A03'),
    ('3A04', '3A04'),
    ('3A05', '3A05'),
    ('3A06', '3A06'),
    ('3A07', '3A07'),
    ('3A08', '3A08'),
    ('3A09', '3A09'),
    ('3A10', '3A10'),
    ('3A11', '3A11'),
    ('3A12', '3A12'),
    ('3B01', '3B01'),
    ('3B02', '3B02'),
    ('3B03', '3B03'),
    ('3B04', '3B04'),
    ('3B05', '3B05'),
    ('3B06', '3B06'),
    ('3B07', '3B07'),
    ('3B08', '3B08'),
    ('3B09', '3B09'),
    ('3B10', '3B10'),
    ('3B11', '3B11'),
    ('3B12', '3B12'),
    ('3C01', '3C01'),
    ('3C02', '3C02'),
    ('3C03', '3C03'),
    ('3C04', '3C04'),
    ('3C05', '3C05'),
    ('3C06', '3C06'),
    ('3C07', '3C07'),
    ('3C08', '3C08'),
    ('3C09', '3C09'),
    ('3C10', '3C10'),
    ('3C11', '3C11'),
    ('3C12', '3C12'),
    ('3D01', '3D01'),
    ('3D02', '3D02'),
    ('3D03', '3D03'),
    ('3D04', '3D04'),
    ('3D05', '3D05'),
    ('3D06', '3D06'),
    ('3D07', '3D07'),
    ('3D08', '3D08'),
    ('3D09', '3D09'),
    ('3D10', '3D10'),
    ('3D11', '3D11'),
    ('3D12', '3D12'),
    ('3E01', '3E01'),
    ('3E02', '3E02'),
    ('3E03', '3E03'),
    ('3E04', '3E04'),
    ('3E05', '3E05'),
    ('3E06', '3E06'),
    ('3E07', '3E07'),
    ('3E08', '3E08'),
    ('3E09', '3E09'),
    ('3E10', '3E10'),
    ('3E11', '3E11'),
    ('3E12', '3E12'),
    ('3F01', '3F01'),
    ('3F02', '3F02'),
    ('3F03', '3F03'),
    ('3F04', '3F04'),
    ('3F05', '3F05'),
    ('3F06', '3F06'),
    ('3F07', '3F07'),
    ('3F08', '3F08'),
    ('3F09', '3F09'),
    ('3F10', '3F10'),
    ('3F11', '3F11'),
    ('3F12', '3F12'),
    ('4A01', '4A01'),
    ('4A02', '4A02'),
    ('4A03', '4A03'),
    ('4A04', '4A04'),
    ('4A05', '4A05'),
    ('4A06', '4A06'),
    ('4A07', '4A07'),
    ('4A08', '4A08'),
    ('4A09', '4A09'),
    ('4A10', '4A10'),
    ('4A11', '4A11'),
    ('4A12', '4A12'),
    ('4B01', '4B01'),
    ('4B02', '4B02'),
    ('4B03', '4B03'),
    ('4B04', '4B04'),
    ('4B05', '4B05'),
    ('4B06', '4B06'),
    ('4B07', '4B07'),
    ('4B08', '4B08'),
    ('4B09', '4B09'),
    ('4B10', '4B10'),
    ('4B11', '4B11'),
    ('4B12', '4B12'),
    ('4C01', '4C01'),
    ('4C02', '4C02'),
    ('4C03', '4C03'),
    ('4C04', '4C04'),
    ('4C05', '4C05'),
    ('4C06', '4C06'),
    ('4C07', '4C07'),
    ('4C08', '4C08'),
    ('4C09', '4C09'),
    ('4C10', '4C10'),
    ('4C11', '4C11'),
    ('4C12', '4C12'),
    ('4D01', '4D01'),
    ('4D02', '4D02'),
    ('4D03', '4D03'),
    ('4D04', '4D04'),
    ('4D05', '4D05'),
    ('4D06', '4D06'),
    ('4D07', '4D07'),
    ('4D08', '4D08'),
    ('4D09', '4D09'),
    ('4D10', '4D10'),
    ('4D11', '4D11'),
    ('4D12', '4D12'),
    ('4E01', '4E01'),
    ('4E02', '4E02'),
    ('4E03', '4E03'),
    ('4E04', '4E04'),
    ('4E05', '4E05'),
    ('4E06', '4E06'),
    ('4E07', '4E07'),
    ('4E08', '4E08'),
    ('4E09', '4E09'),
    ('4E10', '4E10'),
    ('4E11', '4E11'),
    ('4E12', '4E12'),
    ('4F01', '4F01'),
    ('4F02', '4F02'),
    ('4F03', '4F03'),
    ('4F04', '4F04'),
    ('4F05', '4F05'),
    ('4F06', '4F06'),
    ('4F07', '4F07'),
    ('4F08', '4F08'),
    ('4F09', '4F09'),
    ('4F10', '4F10'),
    ('4F11', '4F11'),
    ('4F12', '4F12'),
    ('5A01', '5A01'),
    ('5A02', '5A02'),
    ('5A03', '5A03'),
    ('5A04', '5A04'),
    ('5A05', '5A05'),
    ('5A06', '5A06'),
    ('5A07', '5A07'),
    ('5A08', '5A08'),
    ('5A09', '5A09'),
    ('5A10', '5A10'),
    ('5A11', '5A11'),
    ('5A12', '5A12'),
    ('5B01', '5B01'),
    ('5B02', '5B02'),
    ('5B03', '5B03'),
    ('5B04', '5B04'),
    ('5B05', '5B05'),
    ('5B06', '5B06'),
    ('5B07', '5B07'),
    ('5B08', '5B08'),
    ('5B09', '5B09'),
    ('5B10', '5B10'),
    ('5B11', '5B11'),
    ('5B12', '5B12'),
    ('5C01', '5C01'),
    ('5C02', '5C02'),
    ('5C03', '5C03'),
    ('5C04', '5C04'),
    ('5C05', '5C05'),
    ('5C06', '5C06'),
    ('5C07', '5C07'),
    ('5C08', '5C08'),
    ('5C09', '5C09'),
    ('5C10', '5C10'),
    ('5C11', '5C11'),
    ('5C12', '5C12'),
    ('5D01', '5D01'),
    ('5D02', '5D02'),
    ('5D03', '5D03'),
    ('5D04', '5D04'),
    ('5D05', '5D05'),
    ('5D06', '5D06'),
    ('5D07', '5D07'),
    ('5D08', '5D08'),
    ('5D09', '5D09'),
    ('5D10', '5D10'),
    ('5D11', '5D11'),
    ('5D12', '5D12'),
    ('5E01', '5E01'),
    ('5E02', '5E02'),
    ('5E03', '5E03'),
    ('5E04', '5E04'),
    ('5E05', '5E05'),
    ('5E06', '5E06'),
    ('5E07', '5E07'),
    ('5E08', '5E08'),
    ('5E09', '5E09'),
    ('5E10', '5E10'),
    ('5E11', '5E11'),
    ('5E12', '5E12'),
    ('5F01', '5F01'),
    ('5F02', '5F02'),
    ('5F03', '5F03'),
    ('5F04', '5F04'),
    ('5F05', '5F05'),
    ('5F06', '5F06'),
    ('5F07', '5F07'),
    ('5F08', '5F08'),
    ('5F09', '5F09'),
    ('5F10', '5F10'),
    ('5F11', '5F11'),
    ('5F12', '5F12'),
    ('6A01', '6A01'),
    ('6A02', '6A02'),
    ('6A03', '6A03'),
    ('6A04', '6A04'),
    ('6A05', '6A05'),
    ('6A06', '6A06'),
    ('6A07', '6A07'),
    ('6A08', '6A08'),
    ('6A09', '6A09'),
    ('6A10', '6A10'),
    ('6A11', '6A11'),
    ('6A12', '6A12'),
    ('6B01', '6B01'),
    ('6B02', '6B02'),
    ('6B03', '6B03'),
    ('6B04', '6B04'),
    ('6B05', '6B05'),
    ('6B06', '6B06'),
    ('6B07', '6B07'),
    ('6B08', '6B08'),
    ('6B09', '6B09'),
    ('6B10', '6B10'),
    ('6B11', '6B11'),
    ('6B12', '6B12'),
    ('6C01', '6C01'),
    ('6C02', '6C02'),
    ('6C03', '6C03'),
    ('6C04', '6C04'),
    ('6C05', '6C05'),
    ('6C06', '6C06'),
    ('6C07', '6C07'),
    ('6C08', '6C08'),
    ('6C09', '6C09'),
    ('6C10', '6C10'),
    ('6C11', '6C11'),
    ('6C12', '6C12'),
    ('6D01', '6D01'),
    ('6D02', '6D02'),
    ('6D03', '6D03'),
    ('6D04', '6D04'),
    ('6D05', '6D05'),
    ('6D06', '6D06'),
    ('6D07', '6D07'),
    ('6D08', '6D08'),
    ('6D09', '6D09'),
    ('6D10', '6D10'),
    ('6D11', '6D11'),
    ('6D12', '6D12'),
    ('6E01', '6E01'),
    ('6E02', '6E02'),
    ('6E03', '6E03'),
    ('6E04', '6E04'),
    ('6E05', '6E05'),
    ('6E06', '6E06'),
    ('6E07', '6E07'),
    ('6E08', '6E08'),
    ('6E09', '6E09'),
    ('6E10', '6E10'),
    ('6E11', '6E11'),
    ('6E12', '6E12'),
    ('6F01', '6F01'),
    ('6F02', '6F02'),
    ('6F03', '6F03'),
    ('6F04', '6F04'),
    ('6F05', '6F05'),
    ('6F06', '6F06'),
    ('6F07', '6F07'),
    ('6F08', '6F08'),
    ('6F09', '6F09'),
    ('6F10', '6F10'),
    ('6F11', '6F11'),
    ('6F12', '6F12'),
    ('AT01', 'AT01'),
    ('AT02', 'AT02'),
    ('AT03', 'AT03'),
    ('AT04', 'AT04'),
)


Grade_Choices = (
    ('1학년', '1학년'),
    ('2학년', '2학년'),
    ('3학년', '3학년'),
    ('4학년', '4학년'),
    ('5학년', '5학년'),
    ('6학년', '6학년'),
)

Sex_Choices = (
    ('남', '남'),
    ('여', '여'),
)

School_Choices = (
    ('초등학교', '초등학교'),
    ('중학교', '중학교'),
    ('고등학교', '고등학교'),
)


class School(models.Model):
    name = models.CharField(max_length=15)
    grade = models.CharField(max_length=10, choices=School_Choices)

    class Meta:
        unique_together = (('name', 'grade'),)
        ordering = ['name', 'grade', ]

    def __str__(self):
        return '{}{}'.format(self.name, self.grade)


class Student(models.Model):
    status = models.CharField(max_length=10, choices=STATUS_CHOICES)
    type_choices = models.CharField(
        max_length=20, choices=TYPE_CHOICES, blank=True)
    type_detail_choices = models.CharField(
        max_length=20, choices=TYPE_DETAIL_CHOICES, blank=True)
    cur_class = models.CharField(
        max_length=10, choices=CLASS_CHOICES, blank=True)
    last_class = models.CharField(
        max_length=10, choices=CLASS_CHOICES, blank=True)
    kor_name = models.CharField(max_length=10)
    eng_name = models.CharField(max_length=15, blank=True)
    school = models.ForeignKey(School, on_delete=models.CASCADE)
    grade = models.CharField(max_length=10, choices=Grade_Choices)
    sex = models.CharField(max_length=5, choices=Sex_Choices)
    cell_phone = models.CharField(max_length=20, blank=True)
    cell_m = models.CharField(max_length=20)
    cell_d = models.CharField(max_length=20, blank=True)
    get_sms = models.BooleanField(default=True)
    si = models.CharField(max_length=20, blank=True)
    gu = models.CharField(max_length=20, blank=True)
    dong = models.CharField(max_length=20, blank=True)
    detail_address = models.CharField(max_length=30, blank=True)
    want_class = models.CharField(
        max_length=10, choices=CLASS_CHOICES, blank=True)
    registration_date = models.DateField(null=True, blank=True)
    quit_date = models.DateField(null=True, blank=True)

    class Meta:
        ordering = ['status', 'cur_class', 'kor_name', ]
        unique_together = (('kor_name', 'eng_name', 'cell_m'),)

    def __str__(self):
        return '{} {} ({})'.format(self.cur_class, self.kor_name, self.eng_name)

    def get_absolute_url(self):
        return reverse('students:student-detail', args=[self.pk])

    def get_success_url(self):
        return reverse('students:student-detail', args=[self.pk])


class Consulting(models.Model):
    student = models.ForeignKey(Student, on_delete=models.CASCADE)
    counselor = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    title = models.CharField(max_length=30)
    content = models.TextField()
    when_consult = models.DateField(auto_now=True)

    class Meta:
        ordering = ['when_consult', 'student', ]

    def __str__(self):
        return '{} - {}'.format(self.student, self.title)

    def get_success_url(self):
        return reverse('students:consult-detail', args=[self.pk])
    
    def get_absolute_url(self):
        return reverse('students:consult-detail', args=[self.pk])